...
def build_features(df):
    ...
    for window in [5, 10, 21, 63]:
        df[f'ma_r_{window}'] = df['log_ret'].rolling(window=window, min_periods=1).mean()
    
    # Multi-scale volatility (annualized)
    for window in [5, 10, 21, 63, 126]:
        df[f'vol_{window}'] = df['log_ret'].rolling(window=window, min_periods=1).std() * np.sqrt(252)
    
    # Momentum, skewness, kurtosis
    for window in [21, 63]:
        df[f'mom_{window}'] = df['log_ret'].rolling(window=window, min_periods=1).sum()
        df[f'skew_{window}'] = df['log_ret'].rolling(window=window, min_periods=1).skew()
        df[f'kurt_{window}'] = df['log_ret'].rolling(window=window, min_periods=1).kurt()
    ...
    df['atr'] = (tr.rolling(14, min_periods=1).mean().values / df['adj_close'].values)
    
    # Volume features
    df['vol_ratio_21'] = df['volume'] / df['volume'].rolling(21, min_periods=1).mean()
    
    # Feature list
    feature_cols = ['log_ret', 'abs_ret', 'ma_r_5', 'ma_r_21', 'vol_5', 'vol_21', 
                   'vol_63', 'mom_21', 'skew_21', 'kurt_21', 'atr', 'vol_ratio_21']
...
def main():
    ...
    if 15 < stability['mean_duration_days'] < 90:
