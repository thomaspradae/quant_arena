...
def build_features(df):
    ...
    # LONGER-TERM moving averages (capture persistent trends)
    for window in [21, 63, 126]:
        df[f'ma_r_{window}'] = df['log_ret'].rolling(window=window, min_periods=max(10, window//2)).mean()
    
    # LONGER-TERM volatility (capture regime shifts, not daily noise)
    for window in [21, 63, 126, 252]:
        df[f'vol_{window}'] = df['log_ret'].rolling(window=window, min_periods=max(10, window//2)).std() * np.sqrt(252)
    
    # Momentum, skewness, kurtosis (medium to long term)
    for window in [63, 126]:
        df[f'mom_{window}'] = df['log_ret'].rolling(window=window, min_periods=max(10, window//2)).sum()
        df[f'skew_{window}'] = df['log_ret'].rolling(window=window, min_periods=max(10, window//2)).skew()
        df[f'kurt_{window}'] = df['log_ret'].rolling(window=window, min_periods=max(10, window//2)).kurt()
    
    # ATR (longer window for regime-level volatility)
    high_low = df['high'] - df['low']
    high_close = (df['high'] - df['adj_close'].shift(1)).abs()
    low_close = (df['low'] - df['adj_close'].shift(1)).abs()
    tr = pd.concat([high_low, high_close, low_close], axis=1).max(axis=1)
    df['atr'] = (tr.rolling(21, min_periods=10).mean().values / df['adj_close'].values)
    
    # Volume features (longer term)
    df['vol_ratio_63'] = df['volume'] / df['volume'].rolling(63, min_periods=30).mean()
    
    # VIX-like features: realized vol vs longer-term vol
    realized_vol_21 = df['log_ret'].rolling(21, min_periods=10).std() * np.sqrt(252)
    realized_vol_126 = df['log_ret'].rolling(126, min_periods=60).std() * np.sqrt(252)
    df['vol_regime'] = realized_vol_21 / realized_vol_126
    
    # Feature list (ONLY LONGER-TERM FEATURES)
    feature_cols = ['ma_r_21', 'ma_r_63', 'ma_r_126', 
                   'vol_21', 'vol_63', 'vol_126', 'vol_252',
                   'mom_63', 'mom_126', 
                   'skew_63', 'kurt_63', 
                   'atr', 'vol_ratio_63', 'vol_regime']
